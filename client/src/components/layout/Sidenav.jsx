...
function Sidenav({ color }) {
  ...
  const [role, setRole] = useState('anonymous'); // Default is 'anonymous'
  ...
  useEffect(() => {
    ...
    if (storedRoles) {
      const rolesArray = storedRoles.split(','); // If using a comma-separated string
            ...
            <Menu.Item key="6">
              <NavLink to="/sign-in" onClick={() => {
                ...
                <span
                  className="icon"
                  style={{
                    background: page === "sign-out" ? color : "",
                  }}
                >
                  {profile}
                </span>
